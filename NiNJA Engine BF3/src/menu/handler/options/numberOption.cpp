...
void NumberOption<Type>::Render(int pos) {
	...
	case TOGGLE: {
		...
		pShapes.Rectangle(756 - buffer, pMenu.iYPositonEditable + (360 - (pRenderer.fBgPosition / 2) + pRenderer.fPositionAnimFix) + (pos * 22) + 21, 80, 3, JustifyLeft, GColor(140, 140, 140)); // grey bar
		pShapes.Rectangle(756 + (float)((*pNumber * 78) / Max) - buffer + 1, pMenu.iYPositonEditable + (360 - (pRenderer.fBgPosition / 2) + pRenderer.fPositionAnimFix) + (pos * 22) + 21 - 5, 1, 13, JustifyLeft, pRenderer.UIColors.slider); // blue bar (value scroller)
		...
		pShapes.RectangleOutlined(640 + (410 / 2) - 20, pMenu.iYPositonEditable + (360 - (pRenderer.fBgPosition / 2) + pRenderer.fPositionAnimFix) + (pos * 22) + 17, 10, 10, 1, colors[0], colors[1], true);
		break;
	}

	case SCROLL:
	case SCROLLSELECT:
		pShapes.Rectangle(756, pMenu.iYPositonEditable + (360 - (pRenderer.fBgPosition / 2) + pRenderer.fPositionAnimFix) + (pos * 22) + 21, 80, 3, JustifyLeft, GColor(140, 140, 140)); // grey bar
		pShapes.Rectangle(756 + (float)((*pNumber * 78) / Max) + 1, pMenu.iYPositonEditable + (360 - (pRenderer.fBgPosition / 2) + pRenderer.fPositionAnimFix) + (pos * 22) + 21 - 5, 1, 13, JustifyLeft, pRenderer.UIColors.slider); // blue bar (value scroller)
