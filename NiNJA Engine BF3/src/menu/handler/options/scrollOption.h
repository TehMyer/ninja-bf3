...
private:
	...
	function<void(ScrollOption*)> FuncOnUpdate;
...
public:
	ScrollOption(ScrollOptionType type, const char* name) 
		: Option(name) {
		ScrollType = type;
		bHasMin = false;
		bCanBeSaved = false;
		...
		iScrollSpeed = 100;
		FuncOnAction = [] {};
		FuncOnUpdate = [] (ScrollOption* option) {};
		FuncCurrentOption = [] {};
	}
	...
	ScrollOption& addOnUpdate(function<void()> update) {
		FuncOnUpdate = [=](ScrollOption* option) { update(); };
		return *this;
	}
	...
	ScrollOption& addOnHover(function<void()> func) {
