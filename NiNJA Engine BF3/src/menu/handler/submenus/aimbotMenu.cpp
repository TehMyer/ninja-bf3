...
namespace AimbotMenuVars {
	...
	void AutowallDisable() {
		if (!vars.bAutoWall) {
			for (int i = 0; i < 80; i++) pAimbot.szWallbangable[i] = false;
		}
	}
}
...
void AimbotMenu::Init() {
	...
	addOption(ScrollOption<int>(SCROLL, "Target Bone").canLoop()
...
void AimbotMenu::FeatureUpdate() {
	if (vars.bDrawFOV) {
		...
		pShapes.Circle(center, vars.fFOVRadius, GColor(255, 255, 255), 30); // usually 30 for sides
	}
}
