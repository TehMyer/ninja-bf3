	...
	bool HookFunction(DWORD dwAddress, void* pHookFunction, T* pTrampoline) {
		if (dwAddress) {
			...
			*pTrampoline = (T)startStub;
			...
			DbgPrint("Hooked: Address=0x%X Stub=0x%X", dwAddress, *pTrampoline);
			return true;
		}
	...
	bool HookVirtual(DWORD dwVTable, void* pReplacement, T* pTrampoline) {
		if (dwVTable) {
			...
			*pTrampoline = (T)originalCall;
			...
			DbgPrint("Hooked: Address=0x%X Stub=0x%X", dwVTable, *pTrampoline);
			return true;
		}
...
public:
	...
	void Branch(DWORD dwFrom, DWORD dwTo);

	void Unhook(DWORD dwAddress);
	void UnhookAll();
};

extern BranchHooking pBranchHooking;
